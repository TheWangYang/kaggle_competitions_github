...
class VGG16(torch.nn.Module):
    def __init__(self, num_classes):
        ...
        self.block_1 = nn.Sequential(
            ...
            # 添加一个反卷积层，为了防止图像像素较小，还没有达到fc层之前就输出为dim x width x height，其中后两个参数为0
            nn.ConvTranspose2d(16, 64, kernel_size=(3, 3), stride=(2, 2), bias=False),
            # 继续卷积
        ...
        # 初始化网络权重可能会出现训练过程中train_loss和val_loss为nan的情况
        # # 对CONV2d层添加了权重初始化
...
def create_model(parser_data):
    model = VGG16(num_classes=parser_data.num_classes)
    return model
...
def train_model(parser_data):
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for data in train_loader:
            ...
            # print("[INFO] outputs: {}".format(outputs))
    ...
    DataFrame.to_csv('./submission/{}_resnet_submit.csv'.format(save_date), index=False, sep=',')
